...
const EditJobForm = ({ job }: EditJobFormProps) => {
  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    startTransition(async () => {
      try {
        await fetchClient(`/api/recruiter/jobs/${job._id}`, {
          method: "PATCH",
          body: JSON.stringify(updates),
        });
        toast.success("Job updated successfully!");
        setTimeout(() => router.push("/dashboard/recruiter/my-jobs"), 1500);
      } catch (err) {
        const msg =
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              required
              className="flex h-10 w-full rounded-xl border border-Border dark:border-secondary bg-Background dark:bg-dark-bg px-3 py-2 text-sm font-SecondaryFont text-TextPrimary dark:text-surface outline-none focus:border-PrimaryColor focus:ring-2 focus:ring-PrimaryColor/20 transition-colors cursor-pointer"
            >
              ...
              {requirements.map((req, index) => (
                ...
                >
                  ...
                  <button
                    ...
                    className="shrink-0 p-1 rounded-lg text-TextMuted hover:text-PrimaryColor hover:bg-PrimaryColorLight dark:hover:bg-PrimaryColorDark/20 transition-colors opacity-0 group-hover:opacity-100 cursor-pointer"
                  >
                    <X size={14} />
                  </button>
                </div>
