...
const StatCard = ({ label, value, icon: Icon, gradient, bgLight }: StatCardProps) => {
  return (
    <div className="relative group rounded-2xl bg-white dark:bg-[#1e293b] border border-Border dark:border-secondary p-5 transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5">
      ...
      <div className={`absolute bottom-0 left-0 right-0 h-0.5 rounded-full bg-gradient-to-r ${gradient} scale-x-0 group-hover:scale-x-100 transition-transform duration-500`} />
    </div>
  );
};
